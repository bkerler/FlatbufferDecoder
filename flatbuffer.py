'''
    Flatbuffer decoder and flatbuffer-to-pseudo-xml converter
    (c) B.Kerler 2017
    Confirmed to work on google test data and facebook app data (newsfeed)
'''
...
def main():
    ...
    #Try me for flatbuffer to pseudo-xml:
    print(ft.flattoxml())
    
    #Decoding example, based on google test data
    rootoffset=ft.getint(0)
    roottbl=ft.table_offsets(rootoffset)

    mana=ft.getshort(roottbl[2])
    name=ft.sstring(roottbl[3])

    vec=ft.svector_offsets(roottbl[7])
    tbl7=ft.table_offsets(vec[0])

    #tbl9=ft.table_offsets(ft.soffset(roottbl[9]))
    tbl9=ft.stable_offsets(roottbl[9])
